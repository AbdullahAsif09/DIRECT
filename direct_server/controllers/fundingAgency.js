...
class fundingAgencyController {
  ...
  async verify(req, res) {
    try {
      ...
      var user = await this.fundingAgency.findOne({ _id: Id });
  ...
  async createFundingAgency(req, res) {
    try {
      console.log(req.body);
      const oldUser = await this.fundingAgency.findOne({
        $or: [
      ...
      const user = await this.fundingAgency.create({
        ...req.body,
        role: "FundingAgency",
      });
      user.password = await this.fundingAgency.CreateHash(user.password);
      user.email = req.body.email.toLowerCase();

      SendEmail(user.email, user.firstname, user, res, "fundingAgency");
    } catch (error) {
      console.log(error);
      res
        .status(500)
  ...
  async login(req, res) {
    try {
      var user = await this.fundingAgency.findOne({
        $or: [
          { email: req.body.email.toLowerCase() },
      ...
      if (!user) {
        ...
        const isEqual = await this.fundingAgency.isPasswordEqual(
          req.body.password,
          user.password
        );

        if (isEqual) {
          const token = getToken(user, "fundingAgency", req);
  ...
  async getAllUsers(req, res) {
    try {
      ...
      const userFundingAgency = await this.fundingAgency.aggregate([
        {
          $match: {
...
module.exports = fundingAgencyController;
