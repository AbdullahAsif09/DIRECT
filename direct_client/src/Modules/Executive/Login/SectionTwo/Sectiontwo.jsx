...
function Sectiontwo({ emailsent, setEmailsent }) {
  ...
  const handleLogin = async (values) => {
    try {
      const valuess = { ...values, type: "executive" };

      const data = await API({
        method: "POST",
        url: "executive/signin",
        object: valuess,
      });
      ...
      if (data?.type === "success") {
        ...
        dispatch(
          setAlert({
            status: "error",
            text: data?.response?.data?.result ?? data?.result ?? "Couldn't SendEmail!",
          })
        );
      }
    } catch (e) {
                ...
                <Grid item xs={12}>
                  <Grid container justifyContent={"center"} alignItems={"center"}>
                    <Grid item xs={6}>
                      <FlexBox>
                        {/* <LoginText> */}
                        <LoginLink onClick={() => navigate("/directportal/admin/resetpassword")}>
                          Forgot Password?
                        </LoginLink>
                        {/* </LoginText> */}
                      </FlexBox>
                ...
                <Grid item xs={7}>
                  <FlexBox>
                    <LoginText>
                      ...
                      <LoginLink onClick={() => navigate(route)}>Signup</LoginLink>
                    </LoginText>
                  </FlexBox>
                </Grid>
              </Grid>
