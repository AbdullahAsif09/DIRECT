...
function UploadDocument({ proposalID, pastDocument, urlAwarded, urlContracted }) {
  const dispatch = useDispatch();
  const { API } = useAxios();
  const { id: projectID } = useParams();
  const [DocumentUpload, setDocumentUpload] = useState(null);
  const [prevDocumentUploaded, setPrevDocumentUploaded] = useState(pastDocument);
  const UploadNewFile = async () => {
    ...
    try {
      ...
      if (response.type == "success") {
        ...
        dispatch(setAlert({ text: "Document has been Uploaded", status: "success" }));
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <Grid container gap={5}>
      ...
      {prevDocumentUploaded && (
        <Grid item xs={12}>
          <TypographyMUI variant={"h4"}>Download Previous Uploaded Document</TypographyMUI>
              ...
              <Stack direction={"row"} gap={2}>
                ...
                <Stack direction={"column"} justifyContent={"space-evenly"}>
                  {prevDocumentUploaded?.name ? (
                    <Typography variant={"h6"}>{prevDocumentUploaded?.name}</Typography>
                  ) : null}
              ...
              <Button
                ...
                href={
                  prevDocumentUploaded?.url
                    ...
                    : prevDocumentUploaded instanceof Blob || prevDocumentUploaded instanceof File
